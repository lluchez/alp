; Author : KarLKoX
; Mail   : KarLKoX@gmail.com (for bugs/suggestions)
; Infos  : Read/Write Tags 1.x/2.x
;          Get mp3 infos from header (vbr supported)
; Version:
;  1.01 : (30/07/05)
;       - bugfix with idtag v2.3 (TCON ID) 
;        
;  1.0  : (29/09/03)
;       - Initial Release
...
Protected m_identifiant.s
  ...
  ; loop until all frames = size of the tag
    While m_taille < m_taille2
        ...
        If m_identifiant <> "NULL"
            ...
            ; Debug "ReadFrames::ID = " + UCase(m_identifiant) + " data = " + PeekS(frame\datas) + " m_taille3 = " + Str(m_taille3)
...
Protected m_identifiant.s
    ...
    ; There are 12 ID, note that you can add as much ID as you need (see InitTagV2())
   ...
   *m_frame\datas  = Space(m_taille + 1)
      bytesread = ReadData(@*m_frame\datas, m_taille - 1)
      If (bytesread <> m_taille - 1) Or (bytesread <= 0)
       ;Debug "An error occured while reading frame data"
      EndIf
      ; is there a null terminating string ? If not, add it
      If PeekS(*m_frame\datas) <> Chr(0)
       PokeS(@*m_frame\datas + m_taille - 1, Chr(0))
     EndIf
      ;Debug "ReadFrame::ID = " + m_identifiant + " data = " + PeekS(*m_frame\datas) + " taille = " + Str(m_taille) + " bytesread = " + Str(bytesread)
  EndIf

  If b_Continuer = #False
